...
import { IDynamicsolutionProps, IDynamicsolutionState } from '../interfaces/IDynamicsolution';
import "@pnp/sp/webs";
import "@pnp/sp/site-users/web";
import { BaseService } from '../services/BaseService';
import { Dialog, DialogType, Dropdown, FontWeights, IDropdownOption, IDropdownStyles, IIconProps, IconButton, Label, Modal, PrimaryButton, TextField, Toggle, getTheme, mergeStyleSets } from '@fluentui/react';
...
export default class Dynamicsolution extends React.Component<IDynamicsolutionProps, IDynamicsolutionState> {
  BaseService: any;
  public constructor(props: IDynamicsolutionProps) {
    ...
    this.onNewTabToggleChange = this.onNewTabToggleChange.bind(this);
  }
    ...
    await this.BaseService.getListItemsWithFilterExpand(url, this.props.QuicklinkTypelistName, "Title eq '" + this.props.context.instanceId + "'")
      .then(async (quiklinks: any) => {
        if (quiklinks.length > 0) {
          ...
          const resultString = tempAudienceTargetString.map((item: { label: any; }) => item.label).join(',');
  ...
  public checkingcurrentUserDept = async (AudienceTargets: any, type: string) => {
  ...
  public getAdmin() {
    this.BaseService.getAdmin(this.props.AdminlistName, this.props.siteUrl)
      .then((adminItems: any) => {
        const isAdmin = adminItems.some((item: { Admin: { EMail: string; }; }) => item.Admin.EMail === this.props.context.pageContext.user.email);
  ...
  private showModal = (cards: any, key: any) => {
    this.setState({
      ...
      text: cards.Title,
      link: cards.link,
      OpenInNewTab: cards.OpenInNewTab,
      ID: cards.ID,
      file: cards.file,
        ...
        .then((item: any) => {
          if (this.state.QuicklinktypeID === "Image") {
            updateItem = {
              ...
              OpenInNewTab: item.OpenInNewTab === this.state.OpenInNewTab ? item.OpenInNewTab : this.state.OpenInNewTab,
  ...
  //To open link in new browser tab
  public onNewTabToggleChange(ev: React.MouseEvent<HTMLElement>, checked: boolean) {
            ...
            <div className={styles.iconmain} >
              {this.state.quicklinkitems.map((cards: any, key: any) => {
                ...
                return (
                  ...
                  <div onClick={() => this.onButtonClick(cards.link)} key={key} className={styles.iconcard} style={{ width: this.state.Width !== "" ? this.state.Width : "8rem", height: this.state.Height !== "" ? this.state.Height : "11rem", display: display, backgroundColor: cards.Title.toLowerCase() === "more apps" ? "#0071CE" : "White" }} >

                    <div className={styles.iconImgDiv} style={{ display: cards.Title.toLowerCase() === "more apps" ? "none" : "", }}>
                      <img className={styles.iconimg} src={cards.CardIcon.Url !== null ? cards.CardIcon.Url : "Image"} alt='' />
                    </div>
                    <div className={styles.texticondiv} >
                      <h5 className={styles.iconcardtitle} style={{ color: cards.Title.toLowerCase() !== "more apps" ? "#000000" : "#FFFFFF", padding: cards.Title.toLowerCase() === "more apps" ? "0px 40px" : "" }}>{cards.Title}</h5>
                    </div>
        ...
        >
          ...
          {this.state.editAvlble &&
            < div className={styles.editmodal} >
              ...
              <Toggle label="Open in new tab?" checked={this.state.OpenInNewTab} onText="New Tab" offText="Current tab" onChange={this.onNewTabToggleChange} />
              <div >
                <Label>Audience Target</Label>
